...
### 3. 사이드바 네비게이션
- 대시보드, 피플, 일일촬영계획표, 파일, 설정 (한국어)
- **프로젝트 선택 드롭다운** - 프로젝트 간 빠른 전환
...
## 미구현 기능
- [ ] 다크모드 (CSS 준비 완료, UI 개선 필요)
- [ ] 조직 관리 (멤버 초대/삭제, 권한 설정)
- [ ] 알림 설정
- [ ] 씬 관리 페이지
- [ ] 로케이션 관리 페이지
...
### 2026-01-15
- **프로젝트 선택 드롭다운** (사이드바)
  - 사이드바 상단 프로젝트 이름 클릭 시 드롭다운 표시
  - 현재 조직의 모든 프로젝트 목록 표시
  - 현재 프로젝트 체크 표시
  - 프로젝트 클릭 시 해당 프로젝트로 이동
  - "모든 프로젝트 보기" 링크 → 조직 페이지로 이동

- **사용자 메뉴 드롭다운** (Topbar)
  - 오른쪽 상단 사용자 아이콘 클릭 시 드롭다운 표시
  - 사용자 이름, 이메일 표시
  - "내 정보" → 프로필 설정 페이지로 이동
  - "로그아웃" → 세션 종료 후 로그인 페이지로 이동

- **내 정보 페이지** (`/settings/profile`)
  - 이메일 (읽기 전용), 이름, 연락처 수정 가능
  - 비밀번호 재설정 이메일 발송 기능
...
- **다크모드** (미완성 - 추후 구현)
  - CSS 변수 기반 다크모드 설정 추가
  - 디자인 개선 필요하여 일단 토글 제거

### 2026-01-14
- **소셜 로그인 제거**
  - Google, 카카오 OAuth 버튼 제거
  - 이메일/비밀번호 로그인만 유지

- **대시보드 실제 데이터 연동**
  - 목업 데이터 → Supabase 실제 데이터로 변경
  - 일일촬영계획표 수, 컷 수, 예정 촬영 수 표시
  - 최근 일일촬영계획표 목록 (최대 5개)

- **OAuth PKCE 플로우 수정** (middleware.ts)
  - root URL에서 ?code= 파라미터 처리
  - exchangeCodeForSession으로 세션 생성

### 2026-01-13
- **씬 일괄 추가 기능**
  - "씬 일괄 추가" 모달 (씬 번호, 컷 수, M/D/E/N, I/E, 장소 입력)
  - 입력한 설정으로 여러 컷 한번에 생성

- **같은 씬에 적용 기능**
  - 컷 옆 복사 버튼 클릭 → 같은 씬의 모든 컷에 속성 일괄 적용
  - M/D/E/N, I/E, 장소 복사

- **시간 자동채우기 byRange 모드**
  - 시작/끝 시간 지정 → 자동으로 시간 균등 배분
